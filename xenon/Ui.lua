			...
			if Input.KeyCode == window.key then
					if toggled then
						...
						if window.x == false and window.y == false then
							...
							ts:Create(outline, TweenInfo.new(0,Enum.EasingStyle.Quad,Enum.EasingDirection.In), {Position = UDim2.new(xx,xxx,yy,yyy)}):Play()
						end
					else
						toggled = not toggled
						if window.x == false and window.y == false then
							...
							ts:Create(outline, TweenInfo.new(0,Enum.EasingStyle.Quad,Enum.EasingDirection.Out), {Position = saved}):Play()
						end
					end
				end
			end
